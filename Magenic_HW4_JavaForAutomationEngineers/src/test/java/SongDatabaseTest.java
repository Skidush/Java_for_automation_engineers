...
public class SongDatabaseTest {
    ...
    public void testAddDeleteSong() {
        // TODO FOR HOMEWORK: Write a test for both adding and deleting songs from a song database
        String songName = "Revvin' my Cj7";
        String artist = "Summer Salt";

        SongDatabase database = new SongDatabase();
        Song song = new Song(songName, artist);

        database.addSong(song);
        Assert.assertEquals(database.findSongByName(songName).size(), 1);
        Assert.assertTrue(database.findSongByName(songName).contains(song));

        database.deleteSong(song);
        Assert.assertEquals(database.findSongByName(songName).size(), 0);
        Assert.assertFalse(database.findSongByName(songName).contains(song));
    }
